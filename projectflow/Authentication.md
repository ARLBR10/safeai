...
### Who could we fix some of this?

Every request would hit the database for cases like if the is enough credits for a user so why care with Refresh and Access tokens. 

If we still used this flow and hit the database anyways a good way to fix the expiry token when logout would be to create a "FAKE" user id and it would be unique and every time a `Logout in All devices` happened we would change that and when we would check the credits it would be invalid and an `401 Unauthorized` was shot it would logout in every device. But again we are reading the database anyways so WHY have a so complicated thing.